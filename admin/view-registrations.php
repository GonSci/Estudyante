...
<p>This will list students and the courses they registered in.</p>
    ...
    <div class="form-body">
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (empty($first_name) || empty($last_name) || empty($email) || empty($program) || empty($username) || empty($_POST['password'])) {
                echo '<div class="error-message">⚠️ Please fill in all required fields.</div>';
            } else {
                ...
                if ($check_email->num_rows > 0) {
                    echo '<div class="error-message">✉️ Email already exists. Please use a different email.</div>';
                    $check_email->close();
                } else {
                    $check_email->close();
                    ...
                    if ($check_username->num_rows > 0) {
                        ...
                        if (!$stmt) {
                            echo '<div class="error-message">❌ Database prepare failed: ' . $conn->error . '</div>';
                        } else {
                            $stmt->bind_param(
                                "ssssssssssss",
                            ...
                            if ($stmt->execute()) {
                                ...
                                if ($user_stmt->execute()) {
                                    ...
                                    echo '<div class="error-message">❌ User creation failed: ' . $user_stmt->error . '</div>';
                                }
                                $user_stmt->close();
                            } else {
                                echo '<div class="error-message">❌ Execute failed: ' . $stmt->error . '</div>';
                            }
                            $stmt->close();
                        }
                    }
